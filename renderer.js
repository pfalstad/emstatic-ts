...
var renderTextures = [];
// one precomputed edge-conductance texture per grid level (indexed by level, not by the
// 3-per-level renderTextures scheme); see shader-compute-edge-weights-fs / shader-restrict-edge-weights-fs
var edgeWeightTextures = [];
var brightness;
var renderer = {};
...
const MT_DIELECTRIC = 3;
    ...
    function initShaders() {
    	...
    	shaderProgramRelax.parityUniform = gl.getUniformLocation(shaderProgramRelax, "parity");
    	shaderProgramRelax.edgeWeightsUniform = gl.getUniformLocation(shaderProgramRelax, "uEdgeWeights");
    	...
    	shaderProgramResidual.edgeWeightsUniform = gl.getUniformLocation(shaderProgramResidual, "uEdgeWeights");
    	...
    	shaderProgramComputeEdgeWeights = initShader("shader-compute-edge-weights-fs", "shader-vs", null);
    	shaderProgramComputeEdgeWeights.stepSizeXUniform = gl.getUniformLocation(shaderProgramComputeEdgeWeights, "stepSizeX");
    	shaderProgramComputeEdgeWeights.stepSizeYUniform = gl.getUniformLocation(shaderProgramComputeEdgeWeights, "stepSizeY");

    	shaderProgramRestrictEdgeWeights = initShader("shader-restrict-edge-weights-fs", "shader-vs", null);
    	shaderProgramRestrictEdgeWeights.stepSizeXUniform = gl.getUniformLocation(shaderProgramRestrictEdgeWeights, "stepSizeX");
    	shaderProgramRestrictEdgeWeights.stepSizeYUniform = gl.getUniformLocation(shaderProgramRestrictEdgeWeights, "stepSizeY");
    	shaderProgramRestrictEdgeWeights.sourceEdgeWeightsUniform = gl.getUniformLocation(shaderProgramRestrictEdgeWeights, "uSourceEdgeWeights");
...
//    	gl.framebufferRenderbuffer(gl.FRAMEBUFFER, gl.DEPTH_ATTACHMENT, gl.RENDERBUFFER, renderbuffer);
    ...
    renderer.runRelax = function (srcnum, rsnum, resid, parity) {
        ...
        if (!resid)
            gl.uniform1f(prog.parityUniform, parity);
        ...
        var edgeWeightsRT = edgeWeightTextures[Math.floor(rsnum/3)];
        gl.activeTexture(gl.TEXTURE2);
        gl.bindTexture(gl.TEXTURE_2D, edgeWeightsRT.texture);
    	gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.NEAREST);
    	gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.NEAREST);
        gl.uniform1i(prog.edgeWeightsUniform, 2);
    ...
    // edge-weight textures are a separate array (one per level, not the 3-per-level
    // renderTextures scheme), so building/restricting them is a side-trip to a different
    // destination; save and restore whatever destination the caller had bound so callers
    // like writeConductorMaterials keep drawing into the right place afterward.
    function withEdgeWeightDestination(destRT, drawFn) {
        var savedFB = gl.getParameter(gl.FRAMEBUFFER_BINDING);
        var savedHeight = destHeight;

        gl.bindFramebuffer(gl.FRAMEBUFFER, destRT.framebuffer);
        gl.viewport(0, 0, destRT.framebuffer.width, destRT.framebuffer.height);
        destHeight = destRT.framebuffer.height;

        // the draw below only covers the interior (1..height-1), same as every other pass in
        // this file; the border row/column would otherwise be left as uninitialized texture
        // memory (potentially huge or NaN), which a neighbor read during restriction (a coarse
        // vertex next to the domain edge sampling the fine grid's border) would then propagate
        // inward.  clear explicitly to zero conductance (a safe, inert "no connection" default)
        // rather than relying on the app's global clear color, which is (0,0,1,1) here -- fine
        // for the solution/rs textures but not what we want for edge weights.
        ...
        destHeight = savedHeight;
        gl.viewport(0, 0, savedHeight, savedHeight);
    }

    // build this level's edge conductances (E/N/W/S) from its own rasterized cell
    // permittivity (rs texture's blue channel).  only meaningful at the finest grid level;
    // coarser levels get their edge weights via restrictEdgeWeights instead.
    renderer.computeEdgeWeights = function (rsnum) {
    	var level = Math.floor(rsnum/3);
    	var rightSideRT = renderTextures[rsnum];
    	var destRT = edgeWeightTextures[level];

    	withEdgeWeightDestination(destRT, function () {
	        var prog = shaderProgramComputeEdgeWeights;
	        gl.useProgram(prog);
	    	...
	    	gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.NEAREST);
	    	gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.NEAREST);
	        gl.uniform1i(prog.rightSideTextureUniform, 0);
	        gl.uniform1f(prog.stepSizeXUniform, 1/rightSideRT.framebuffer.width);
	        gl.uniform1f(prog.stepSizeYUniform, 1/rightSideRT.framebuffer.height);
    ...
    // restrict this level's edge conductances from the next finer level's edge conductances,
    // via per-axis harmonic mean -- see shader-restrict-edge-weights-fs.  fineRsnum is the
    // finer level's rs texture index (used only to identify the level); its edge weights must
    // already be up to date (computeEdgeWeights for the finest level, or a previous
    // restrictEdgeWeights call).
    renderer.restrictEdgeWeights = function (fineRsnum) {
    	var fineLevel = Math.floor(fineRsnum/3);
    	var coarseLevel = fineLevel - 1;
    	var fineEW = edgeWeightTextures[fineLevel];
    	var destRT = edgeWeightTextures[coarseLevel];

    	withEdgeWeightDestination(destRT, function () {
	        var prog = shaderProgramRestrictEdgeWeights;
	        gl.useProgram(prog);
	        ...
	        gl.activeTexture(gl.TEXTURE0);
	        gl.bindTexture(gl.TEXTURE_2D, fineEW.texture);
	    	gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.NEAREST);
	    	gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.NEAREST);
	        gl.uniform1i(prog.sourceEdgeWeightsUniform, 0);

	        gl.uniform1f(prog.stepSizeXUniform, 1/fineEW.framebuffer.width);
	        gl.uniform1f(prog.stepSizeYUniform, 1/fineEW.framebuffer.height);
    ...
    // debug helper: dump a render texture's channel to the console as a grid of numbers,
    // so multigrid restriction (or anything else) can be sanity-checked from devtools, e.g.:
    //   renderer.dumpChannel(rtnum, 2)   // channel 2 = blue = permittivity/material
    // rows are printed top row first (largest y in world space) to match how the scene looks
    // on screen; each row is one line so you can visually scan for where values change.
    renderer.dumpChannel = function (rtnum, channel, digits) {
        var rt = renderTextures[rtnum];
        var w = rt.framebuffer.width, h = rt.framebuffer.height;
        var oldFB = gl.getParameter(gl.FRAMEBUFFER_BINDING);
        gl.bindFramebuffer(gl.FRAMEBUFFER, rt.framebuffer);
        var pixels = new Float32Array(4*w*h);
        gl.readPixels(0, 0, w, h, gl.RGBA, gl.FLOAT, pixels);
        gl.bindFramebuffer(gl.FRAMEBUFFER, oldFB);
        digits = (digits === undefined) ? 2 : digits;
        var lines = [];
        var y, x;
        // GL row 0 is the bottom of the texture; print top-to-bottom to match the on-screen view
        for (y = h-1; y >= 0; y--) {
            var row = [];
            for (x = 0; x < w; x++)
                row.push(pixels[(y*w+x)*4 + channel].toFixed(digits));
            lines.push(row.join(" "));
        }
        console.log("texture " + rtnum + " (" + w + "x" + h + "), channel " + channel + ":\n" + lines.join("\n"));
        return pixels;
    }

    // debug helper: dump just the middle column of a render texture's channel, top to bottom,
    // e.g. to check a dielectric interface that runs horizontally across the middle of the grid:
    //   renderer.dumpColumn(rtnum, 2)   // channel 2 = blue = permittivity/material
    renderer.dumpColumn = function (rtnum, channel, digits) {
        var rt = renderTextures[rtnum];
        var w = rt.framebuffer.width, h = rt.framebuffer.height;
        var x0 = w >> 1;
        var oldFB = gl.getParameter(gl.FRAMEBUFFER_BINDING);
        gl.bindFramebuffer(gl.FRAMEBUFFER, rt.framebuffer);
        var pixels = new Float32Array(4*h);
        gl.readPixels(x0, 0, 1, h, gl.RGBA, gl.FLOAT, pixels);
        gl.bindFramebuffer(gl.FRAMEBUFFER, oldFB);
        ...
        // GL row 0 is the bottom of the texture; print top-to-bottom to match the on-screen view
        for (y = h-1; y >= 0; y--)
            lines.push("y=" + y + ": " + pixels[y*4 + channel].toFixed(digits));
        console.log("texture " + rtnum + " (" + w + "x" + h + "), column x=" + x0 + ", channel " + channel + ":\n" + lines.join("\n"));
        return pixels;
    }

    // debug helper: sum of |channel| over an entire render texture, e.g. as a residual norm.
    renderer.sumAbsChannel = function (rtnum, channel) {
        var rt = renderTextures[rtnum];
        var w = rt.framebuffer.width, h = rt.framebuffer.height;
        var oldFB = gl.getParameter(gl.FRAMEBUFFER_BINDING);
        gl.bindFramebuffer(gl.FRAMEBUFFER, rt.framebuffer);
        ...
        gl.bindFramebuffer(gl.FRAMEBUFFER, oldFB);
        var i, tot = 0;
        for (i = channel; i < pixels.length; i += 4)
            tot += Math.abs(pixels[i]);
        return tot;
    }

    // debug helper: scan a render texture for NaNs in any channel, report the first one found.
    // returns the count of NaN values found (0 if clean).
    renderer.findNaN = function (rtnum) {
        var rt = renderTextures[rtnum];
        if (!rt)
            return 0;
        var w = rt.framebuffer.width, h = rt.framebuffer.height;
        ...
        gl.readPixels(0, 0, w, h, gl.RGBA, gl.FLOAT, pixels);
        gl.bindFramebuffer(gl.FRAMEBUFFER, oldFB);
        var i, count = 0, first = null;
        for (i = 0; i < pixels.length; i++) {
            if (isNaN(pixels[i])) {
                count++;
                if (!first) {
                    var px = (i/4)|0;
                    first = {x: px % w, y: (px / w)|0, channel: i % 4};
                }
            }
        }
        if (count > 0)
            console.log("texture " + rtnum + " (" + w + "x" + h + "): " + count +
                         " NaNs, first at x=" + first.x + " y=" + first.y + " channel=" + first.channel);
        return count;
    }

    // debug helper: scan every render texture for NaNs and log which ones are affected.
    renderer.scanAllForNaN = function () {
        var i, any = false;
        for (i = 0; i < renderTextures.length; i++) {
            if (renderer.findNaN(i) > 0)
                any = true;
        }
        if (!any)
            console.log("scanAllForNaN: no NaNs found in " + renderTextures.length + " textures");
    }
...
*/
	...
	mat4.rotateX(matrix3d, -Math.PI/3);
    	...
    	renderer.setResolution = function (x, y, wx, wy) {
    		...
    		renderTextures = [];
    		for (var i = 0; i != edgeWeightTextures.length; i++)
    			deleteRenderTexture(edgeWeightTextures[i]);
    		edgeWeightTextures = [];
    		var sz = 8;

                // create 3 of each size, doubling each time, plus one edge-weight texture per level
    		while (1) {
    			console.log("creating buffers size " + sz + " at " + renderTextures.length);
    			renderTextures.push(initTextureFramebuffer(sz));
    			renderTextures.push(initTextureFramebuffer(sz));
    			renderTextures.push(initTextureFramebuffer(sz));
    			edgeWeightTextures.push(initTextureFramebuffer(sz));
    			if (sz >= gridSizeX)
    				break;
    			sz *= 2;
    		}
